...
public class chat {
                ...
                while (true) {
                    try {
                        ...
                        if (bytesRead == -1) {
                            ...
                            if (servSocket != null) {
                                servSocket.close();
                                chat.serverPortsMap.remove(servSocket);
                            }
                            clientSocket.close();
    ...
    private static String isValidConnection(String destination, int port) {
        ...
        if (destination.equals(myIPAddress) && port == chat.serverSocket.getLocalPort()) {
    ...
    private static Socket findServerSocket(Socket clientSocket, int port) {
        ...
        for (Socket ss : serverSocketList) {
            if (clientSocket.getInetAddress().getHostAddress().equals(ss.getInetAddress().getHostAddress())
                    && chat.serverPortsMap.get(ss) == port) {
                return ss;
            }
        }
        return null;
